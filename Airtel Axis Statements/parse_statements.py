#!/usr/bin/env python3
"""
Parse all Airtel Axis Bank Credit Card statement PDFs.
Corrected for actual Axis Bank PDF text format.
"""
...
def parse(text, filename):
    ...
    # --- Summary fields ---
    # Format: "9,403.33   Dr 4,729.00   Dr\n..." after the header labels
    payment_summary = re.search(
        r'(\d{2}/\d{2}/\d{4})\s*-\s*\d{2}/\d{2}/\d{4}\s+'   # Statement period start
        r'(\d{2}/\d{2}/\d{4})\s+'                              # Payment Due Date
        r'\d{2}/\d{2}/\d{4}'                                   # Statement Gen Date
        r'([\d,]+\.\d+)\s*Dr\s+([\d,]+\.\d+)\s*Dr',           # Total Due | Min Due
        text
    )
    ...
    credit_line = re.search(
        r'539494\*+\d{4}\s+([\d,]+\.\d+)\s+([\d,]+\.\d+)\s+([\d,]+\.\d+)',
        text
    )
    ...
    # --- Transactions: DD/MM/YYYY Description Amount Dr/Cr ---
    ...
    for m in tx_pattern.finditer(text):
        date, desc, amount, dr_cr = m.groups()
        transactions.append({
            "date": date,
    ...
    for t in transactions:
        if t['type'] == 'Dr':
            # Try to extract category label from description
...
def main():
    pdf_files = sorted([f for f in os.listdir(PDF_DIR) if f.endswith('.pdf') and 'Airtel_Axis_Statement' in f])
    print(f"Processing {len(pdf_files)} statements...\n")
    ...
    print(f"TOTALS (14 months): Spends=₹{total_spends:,.2f} | CB Earned=₹{total_cb_earned:,.2f} | CB Credited=₹{total_cb_credited:,.2f}")
