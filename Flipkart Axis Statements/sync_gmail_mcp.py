...
ALERTS_FILE = "/Users/ejazanwar/Documents/Gmail Automations/Flipkart Axis Statements/gmail_alerts.json"
STEP_DIR = "/Users/ejazanwar/.gemini/antigravity/brain/33a1c5fd-1c84-4317-92ce-064ddb1aa7a3/.system_generated/steps"
...
def main():
    ...
    if os.path.exists(STEP_DIR):
        for folder in sorted(os.listdir(STEP_DIR), key=lambda x: int(x) if x.isdigit() else 999):
            ...
            # Ensure it is a Flipkart Axis transaction alert
            if "XX6969" not in content or "spent on credit card" not in content:
                continue
            ...
            # Fallback to clean HTML search if direct match returns empty
            if not merchant_name or merchant_name.upper() == "UNKNOWN":
